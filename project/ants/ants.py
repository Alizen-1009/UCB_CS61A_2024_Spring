...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        while(not t.is_hive):
            if(t.bees and self.lower_bound <= cnt <= self.upper_bound):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if(self.turns_to_chew == 0 and list(self.place.bees)):
    ...
    def insects_in_front(self):
        ...
        while(not tmp.is_hive):
            for bee in tmp.bees:
                insect.append(bee)
                distance.append(dis)
            if(tmp.ant and tmp.ant != self):
                insect.append(tmp.ant)
                distance.append(dis)
            dis += 1
            tmp = tmp.entrance
        return dict(zip(insect, distance))
