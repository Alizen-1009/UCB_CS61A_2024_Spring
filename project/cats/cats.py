...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(target):
        target = strip_punctuation(target)
...
def accuracy(typed, source):
    ...
    cnt = 0
    for i in range(0, min(len(source_words), len(typed_words))):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    mi = limit + 1.0
    for s in word_list:
        val = diff_function(typed_word, s, limit)
        if(val <= limit):
            if(val < mi):
                mi = val
                res = s
...
def feline_fixes(typed, source, limit):
    ...
    5
    """
    if(limit < 0) or (not typed and not source):
        return 0
    ...
    if(typed[0] == source[0]):
        flg = 0
    return flg + feline_fixes(typed[1:], source[1:], limit - flg)
...
def report_progress(typed_words, source_words, user_id, upload):
...
def fastest_words(match):
    ...
    [4, 1, 6]
    """
    players= get_all_times(match)  # contains an *index* for each player
