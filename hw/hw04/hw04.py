...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    l_len, l_mass = left_arm[1], total_mass(left_arm[2])
    r_len, r_mass = right_arm[1], total_mass(right_arm[2])
    flg1 = l_len * l_mass == r_len * r_mass
    flg2, flg3 = True, True
    if(is_mobile(left_arm[2])):
        flg2 = balanced(left_arm[2])
    if(is_mobile(right_arm[2])):
        flg3 = balanced(right_arm[2])
